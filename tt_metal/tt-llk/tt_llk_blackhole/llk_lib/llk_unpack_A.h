    ...
    const bool transpose_of_faces, const ckernel::TensorShape tensor_shape, const std::uint32_t unpack_src_format, const std::uint32_t unpack_dst_format = 0)
{
    ...
    else if constexpr (BType == BroadcastType::COL)
    {
        if constexpr (acc_to_dest)
        {
            ...
            tmp.set_end_op(num_faces_c_dim < MAX_NUM_FACES_C_DIM ? TT_OP_SETADCZW(p_setadc::UNP_B, 0, 0, 0, 1, 0b0001) : srcb_set_z_2);
            tmp.program();
        }
        else
        {
 ...
 *
 * Currently a no-op on Blackhole; kept as the symmetric cleanup call for
