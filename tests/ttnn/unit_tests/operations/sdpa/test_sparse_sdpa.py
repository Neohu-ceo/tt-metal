...
"""sparse_sdpa (sparse MLA prefill) — BASIC post-commit smoke (Blackhole only). See PLAN_sparse_sdpa.md.
...
def test_sparse_sdpa_runs(device):
    ...
    out, _ = run_op(q, kv, indices, device, kc)
...
def test_sparse_sdpa_pcc_single_chunk(device, nv_fn, nv_id):
    ...
    out, scale = run_op(q, kv, indices, device, k_chunk_size=TOPK)
...
def test_sparse_sdpa_oversized_persistent_kv(device):
    ...
    for valid_len in (256, 1024):  # each call's queries attend only to the [0, valid_len) populated prefix
        ...
        p = pcc(ttnn.to_torch(tt_out), golden(q, kv, indices, scale))
...
def test_sparse_sdpa_indexed_kv_cache(device):
    ...
    for cb in range(B):
        ...
        p = pcc(ttnn.to_torch(tt_out), golden(q, kv_full[cb : cb + 1], indices, scale))  # golden uses slot cb
...
def test_sparse_sdpa_indexed_nd_sharded_kv(device):
    ...
    for cb in range(B):
        ...
        p = pcc(ttnn.to_torch(tt_out), golden(q, kv_full[cb : cb + 1], indices, scale))
...
def test_sparse_sdpa_nd_sharded_kv_len_change(device):
    ...
    for T in (256, 512):  # multiples of 128; same shard spec, different T (different shard count)
        ...
        p = pcc(ttnn.to_torch(out), golden(q, kv, indices, scale))
