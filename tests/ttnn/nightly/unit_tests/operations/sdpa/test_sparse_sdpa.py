...
"""sparse_sdpa (sparse MLA prefill) — NIGHTLY full coverage (Blackhole only). See PLAN_sparse_sdpa.md.
...
def test_sparse_sdpa_pcc_single_chunk(device, S, T, TOPK, nv_fn, nv_id):
    H = 32
    q, kv, indices = make_inputs(H, S, T, TOPK, nv_fn)
    out, scale = run_op(q, kv, indices, device, k_chunk_size=TOPK)  # single chunk
    p = pcc(out, golden(q, kv, indices, scale))
...
def test_sparse_sdpa_pcc(device, S, T, TOPK, kc, nv_fn, nv_id):
    H = 32
    q, kv, indices = make_inputs(H, S, T, TOPK, nv_fn)
    out, scale = run_op(q, kv, indices, device, kc)
    p = pcc(out, golden(q, kv, indices, scale))
...
def test_sparse_sdpa_multitoken(device, TOPK, kc):
    H, S, T = 32, 256, 512
    q, kv, indices = make_inputs(H, S, T, TOPK, lambda s: 1 + (s * 5) % TOPK)
    out, scale = run_op(q, kv, indices, device, kc)
    p = pcc(out, golden(q, kv, indices, scale))
...
def test_sparse_sdpa_fp8_kv(device, S, T, TOPK, kc, nv_fn, nv_id):
    H = 32
    q, kv, indices = make_inputs(H, S, T, TOPK, nv_fn)
    out, scale = run_op(q, kv, indices, device, kc, kv_dtype=ttnn.fp8_e4m3)
    kv_fp8 = kv.to(torch.float8_e4m3fn).to(torch.float32)  # what the device holds after fp8 quantization
    p = pcc(out, golden(q, kv_fp8, indices, scale))
...
def test_sparse_sdpa_fp8_q(device, S, T, TOPK, kc, kv_dtype, kv_id):
    H = 32
    q, kv, indices = make_inputs(H, S, T, TOPK, lambda s: 1 + (s * 3) % 50)
    out, scale = run_op(q, kv, indices, device, kc, kv_dtype=kv_dtype, q_dtype=ttnn.fp8_e4m3)
    ...
    p = pcc(out, golden(q_fp8, kv_g, indices, scale))
...
def test_sparse_sdpa_query_subblock(device, H, groups):
    ...
    q, kv, indices = make_inputs(H, S, T, TOPK, lambda s: 1 + (s * 3) % 50)
    out, scale = run_op(q, kv, indices, device, kc, compute_kernel_config=cfg)
    p = pcc(out, golden(q, kv, indices, scale))
...
def test_sparse_sdpa_determinism(device, q_dtype, kv_dtype):
    ...
    q, kv, indices = make_inputs(H, S, T, TOPK, lambda s: TOPK)
...
def test_sparse_sdpa_perf(device, S, T, TOPK, kc, nv):
    H = 32
    nv_fn = _NV[nv]
    q, kv, indices = make_inputs(H, S, T, TOPK, lambda s: nv_fn(s, T, TOPK))
    out, _ = run_op(q, kv, indices, device, kc)  # no golden; correctness covered by the PCC tests
