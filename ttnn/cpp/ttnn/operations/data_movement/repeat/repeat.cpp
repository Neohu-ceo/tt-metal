...
ttnn::Tensor repeat(
    ...
    if (!native_sharded) {
        if (input_tensor.memory_config().is_sharded()) {
            // DRAM-sharded: use to_memory_config (sharded_to_interleaved is L1-only).
            const MemoryConfig l1_interleaved{TensorMemoryLayout::INTERLEAVED, BufferType::L1};
            working_tensor = ttnn::to_memory_config(input_tensor, l1_interleaved, std::nullopt);
        }
    ...
    if (!native_sharded && output_mem_config.is_sharded()) {
        ...
        if (!final_mc.shard_spec().has_value()) {
            ...
            auto synth = operations::data_movement::repeat::generate_repeat_shard_spec(
                working_tensor,
                working_tensor.padded_shape(),
                final_mc.memory_layout(),
                /*repeat_dim*/ -1,
                /*num_repeats*/ 1);
            if (synth.has_value()) {
